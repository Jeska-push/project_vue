...
<template>
  ...
  <ul id="componentContainer" :class="paginationContainerClass">
    ...
    <li v-if="showEndingButtons && firstPageButtonIfCondition">
      <component :is="type === 'button' ? 'button' : 'a'" :href="navigationHandler(isRtl ? totalPages : 1)"
        @click.prevent="onClickHandler(isRtl ? totalPages : 1)" :class="[
          firstPageButtonClass,
          paginateButtonsClass,
    ...
    <li v-if="showJumpButtons && startingBreakPointButtonIfCondition">
      <component :is="type === 'button' ? 'button' : 'a'" :href="navigationHandler(
        isRtl
    ...
    <li v-if="!hidePrevNext && backButtonIfCondition">
      <component :is="type === 'button' ? 'button' : 'a'" :href="navigationHandler(isRtl ? currentPageRef + 1 : currentPageRef - 1)
        " @click.prevent="
    ...
    <li v-for="(page, index) in paginate.pages" :key="index">
      <component :is="type === 'button' ? 'button' : 'a'" :href="navigationHandler(page)"
        @click.prevent="() => onClickHandler(page)" :class="[
          paginateButtonsClass,
          numberButtonsClass,
    ...
    <li v-if="showBreakpointButtons && lastButtonIfCondition">
      <component :is="type === 'button' ? 'button' : 'a'" :href="navigationHandler(isRtl ? 1 : totalPages)"
        @click.prevent="onClickHandler(isRtl ? 1 : totalPages)" :class="[
          lastButtonClass,
          paginateButtonsClass,
          disablePagination ? disabledPaginateButtonsClass : '',
